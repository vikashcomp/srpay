...
import { getFundAccept, getFundList, getFundListAdmin } from '../redux/actions/fundRequestAction';
...
const FundApproveCheckerl1 = () => {
    ...
    const { fundRequestListAdmin } = useSelector((state) => state.fundRequestForm);
    console.log('fundRequestListAdmin',fundRequestListAdmin)
    ...
    useEffect(() => {
        dispatch(getFundListAdmin({ access_token: token }))
    }, [])
    ...
    const handlePageChange = (pageNumber) => {
      ...
      const handleAcceptReject = (item, type) => {
        if (type === true) {
          ...
          dispatch(getFundAccept(data)).then((res) => {
            if (res.success == true) {
              dispatch(getFundList({ access_token: token }))
    ...
    const handleApply = () => {
        ...
        if (
          ...
          dispatch(getFundList(data)).then((res) => {
            if (res.status === true) {
                    ...
                    <div className='col-12 main-bg mt-3 mb-3 '>
                        ...
                        <div className="container">
                            <table className="table table-bordered table-striped" ref={tableRef}>
                                ...
                                <tbody>
                                    {
                                        fundRequestListAdmin?.length > 0 ?
                                        fundRequestListAdmin?.map((x,i) => {
                                            return (
                                                <tr key={i}>
...
export default FundApproveCheckerl1;
